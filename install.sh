...
f rule_src="resources/$fname_rule"
f_rule_dest="/etc/udev/rules.d/$fname_rule"
sudo cp "$f_rule_src" "$f_rule_dest" && sudo chmod 311 "$f_rule_dest"
if [ "$?" != '0' ]; then
...
sudo cp "$f_systemd_src" "$f_systemd_dest" && sudo systemctl enable "$fname_systemd"
if [ "$?" != '0' ]; then
  print_error_exit 'Failed to copy systemd config or to enable it!'
fi

# start systemd service to circument reboot
sudo systemd restart "$fname_systemd"
